...
//		}
		...
		void setVerticalHits() {
			...
			for (int i = 0; i < raycastLine; i++) {
				Vector2 orginPos  = new Vector2(baseWidth, self.transform.position.y);
				Vector2 topTargetPos = new Vector2(baseWidth, self.transform.position.y + (1*1.15f));
				Vector2 bottomTargetPos = new Vector2(baseWidth, self.transform.position.y + (-1*1.15f));
		...
		void setSideHits() {
			...
			float baseHeight = self.transform.position.y + (horizontalGap*1.5f);
			for (int i = 0; i < raycastLine; i++) {
				Vector2 orginPos  = new Vector2(self.transform.position.x, baseHeight);
				Vector2 targetPos = new Vector2(self.transform.position.x + (faceSide/1.5f), baseHeight);
